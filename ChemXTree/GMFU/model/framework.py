...
"""Basic framework for Tabular model"""
import warnings
...
from .blocks import heads
from ChemXTree.GMFU.utils import get_logger, reset_all_weights
...
class TablFramework(pl.LightningModule, metaclass=ABCMeta):
    def __init__(
        self,
        config: DictConfig,
        ...
        **kwargs,
        ...
        if not WANDB_INSTALLED:
            ...
            warnings.warn(
                ...
                "You can install wandb using pip install wandb"
            )
    ...
    def calculate_metrics(self, y: torch.Tensor, y_hat: torch.Tensor, tag: str) -> List[torch.Tensor]:
        ...
        Args:
            ...
            tag (str): The tag to use for logging, training/valid/test
...
class _GenericModel(TablFramework):
    def __init__(
        self,
        backbone: nn.Module,
        head: str,
    ...
    def _get_head_from_config(self):
        _head_callable = getattr(heads, self.kwargs.get("head"))
        return _head_callable(
            in_units=self.backbone.output_dim,
            output_dim=self.hparams.output_dim,
