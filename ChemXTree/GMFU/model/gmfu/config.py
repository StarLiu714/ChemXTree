...
from ChemXTree.GMFU.configuration.ModelConfig import ModelConfig


@dataclass
class GateModulationFeatureUnitConfig(ModelConfig):
    """Gate Modulation Feature Unit Ensemble Config

    Args:
        num_tasks (int, optional): Number of tasks. When >1 is multi-class.

        n_heads (int, optional): Number of heads. When >1 is multi-head attention.

        residual_scale (float, optional): Scale of residual layer. 
    ...
    num_tasks: int = field(
        default=1, 
        metadata={"help": "Number of tasks for the model. Defaults to 1"}
    )

    n_heads: int = field(
        default=1, 
        metadata={"help": "Number of heads for the model. Defaults to 1"}
    )

    residual_scale: float = field(
        default=0.05,
        metadata={"help": "Scale of residual layer. . Defaults to 0.05"}
    )
    ...
    _module_src: str = field(default="model.gmfu")
    _model_name: str = field(default="GateModulationFeatureUnitModel")
    _backbone_name: str = field(default="GateModulationFeatureUnitBackbone")
    _config_name: str = field(default="GateModulationFeatureUnitConfig")


if __name__ == "__main__":
    from ChemXTree.GMFU.utils import generate_doc_dataclass
    print(generate_doc_dataclass(GateModulationFeatureUnitConfig, desc="Gate Modulation Feature Unit Ensemble Config"))
