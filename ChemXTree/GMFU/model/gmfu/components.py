...
from ChemXTree.GMFU.model.blocks.activatiors import entmax15
...
class GFLUnit(nn.Module):
    def __init__(
        ...
        dropout: float = 0.0,
        n_heads: int =1,
        residual_scale: float = 0.05  # when n_heads =1, residual scale will not be available
    ):
        super().__init__()
        ...
        self.head_dim = n_features_in//n_heads
        self.residual_scale = residual_scale
        self.multi_head_attention = nn.MultiheadAttention(
            embed_dim=n_features_in, 
            num_heads=n_heads,
            dropout=0.1
            )
        self._build_network()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Multi-head attention
            ...
            attn_out, _ = self.multi_head_attention(h, h, h) 
            h = h + attn_out
            for d in range(self.n_stages):
                feature = self.feature_mask_function(self.feature_masks[d]) * x
                h_in = self.W_in[d](torch.cat([feature, h], dim=-1))
                ...
                h_out = torch.tanh(self.W_out[d](torch.cat([r * h, x], dim=-1)))
                h = self.dropout((1 - z) * h + z * h_out) 
        return h + x * self.residual_scale
